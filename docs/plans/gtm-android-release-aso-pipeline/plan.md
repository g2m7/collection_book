...
- **Deep Linking Engine**:
  - Digital Asset Links (`assetlinks.json`) deployed to `collectionbook.in/.well-known/assetlinks.json`.
  - Route intents:
    - `https://collectionbook.in/import`: Opens MSO import wizard directly.
    - `https://collectionbook.in/r/{ref}`: Captures referral attribution.
...
android {
    defaultConfig {
        applicationId "com.collectionbook.app"
        minSdkVersion 21
        targetSdkVersion 35
        versionCode 100
        versionName "1.0.0"
...
- [ ] **Phase 2: Android App Links Configuration**
  - [ ] Add `intent-filter` with `android:autoVerify="true"` in `android/app/src/main/AndroidManifest.xml`.
  - [ ] Generate SHA-256 fingerprint from release keystore.
  - [ ] Create `assetlinks.json` mapping `collectionbook.in` domain.
  - [ ] Implement Flutter deep link navigation parser in `lib/main.dart`.
...
- Run `adb shell am start -a android.intent.action.VIEW -d "https://collectionbook.in/import" com.collectionbook.app` to test App Link resolution.
