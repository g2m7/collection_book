/**
 * Guard against marketing claims the app does not actually implement.
 *
 * Each entry pairs a stable identifier with a human-readable reason so a
 * future edit that reintroduces an unsupported promise fails loudly. The
 * claim check runs inside the same assertion the generator uses, so a bad
 * claim fails `planListingFiles`, generation, and `--check` before any file
 * is written.
 */
...
const unitPattern =
  "(?:subscribers?|customers?|connections?|users?|" +
...
export interface ClaimIssue {
  ...
  readonly hits: readonly UnsupportedClaimHit[];
}
